...
class PluginFormcreatorForm extends CommonDBTM
{
   ...
   public function showFormList($rootCategory = 0, $keywords = '', $helpdeskHome = false) {
      ...
      if (!empty($keywords)) {
         ...
         //$highWeightedMatch = " MATCH($table_form.`name`, $table_form.`description`) AGAINST('$keywords')";
         $highWeightedMatch = " ($table_form.`name` LIKE '%$keywords%' OR $table_form.`description` LIKE '%$keywords%')";
         //$lowWeightedMatch = " MATCH($table_question.`name`, $table_question.`description`) AGAINST('$keywords')";
         $lowWeightedMatch = " ($table_question.`name` LIKE '%$keywords%' OR $table_question.`description` LIKE '%$keywords%')";
